...
import { TrendingUp, ShieldCheck, Gift, AlertTriangle, Copy, Loader2, Globe, XCircle } from 'lucide-react';
...
const LoginScreen: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Captures: "Firebase: Error (auth/unauthorized-domain)."
      if (
      ...
      // 2. Ignore Popup Closed by User
      if (errorCode === 'auth/popup-closed-by-user' || errorCode === 'auth/cancelled-popup-request') {
         return; 
      }

      // 3. Operation Not Allowed (Google Sign In disabled)
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 relative z-10 text-center animate-fade-up border border-slate-100">
        ...
        {domainError && (
          <div className="mb-6 bg-amber-50 border border-amber-200 p-4 rounded-2xl text-left animate-fade-up ring-4 ring-amber-50/50">
            ...
            <p className="text-[11px] text-slate-700 mb-3 leading-relaxed font-medium">
              Firebase memblokir login dari domain ini. Tambahkan domain berikut ke <strong>Firebase Console &gt; Authentication &gt; Settings &gt; Authorized Domains</strong>.
            </p>
            <div 
              onClick={() => {
                navigator.clipboard.writeText(domainError);
              }}
              className="flex items-center justify-between bg-white border border-amber-200 p-3 rounded-xl cursor-pointer hover:bg-amber-100 transition-colors group shadow-sm"
            >
              <code className="text-xs font-mono text-amber-900 break-all font-bold select-all">{domainError}</code>
              <Copy className="w-4 h-4 text-amber-400 group-hover:text-amber-600" />
            </div>
            <p className="text-[9px] text-amber-500/80 mt-2 italic text-center">Tap kotak kode untuk menyalin domain</p>
          </div>
        )}
        ...
        {/* Feature Pill / Offer (Only show if no error) */}
        ...
        <button 
          ...
          disabled={loading}
          className={`w-full text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-xl active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed group ${
             domainError || generalError ? 'bg-slate-700 hover:bg-slate-800 shadow-slate-200' : 'bg-slate-900 hover:bg-black shadow-slate-200'
          }`}
        >
          {loading ? (
            ...
            <>
              ...
              {domainError || generalError ? "Coba Login Lagi" : "Masuk dengan Google"}
            </>
          )}
        </button>
